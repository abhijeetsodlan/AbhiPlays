...
const SpaceInvaders = () => {
  ...
  const [startGame, setStartGame] = useState(false); // New state for starting the game
  ...
  const shootBullet = () => {
    if (!gameOver) {
      setBullets((prev) => [...prev, { x: spaceshipX + 20, y: window.innerHeight - 110 }]);
    }
  };
    ...
    <div className="min-h-screen bg-black flex flex-col items-center relative text-white">
      {!startGame && !gameOver && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-80">
          ...
          <p className="text-xl mb-5">Use the rocket button to shoot enemies!</p>
          <button
            onClick={startGameHandler}
            className="bg-yellow-400 text-black px-6 py-3 rounded-md text-lg font-bold hover:bg-yellow-50000"
          >
            Start Game
          </button>
        </div>
      ...
      {gameOver && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-80">
          ...
          <button
            onClick={restartGame}
            className="bg- bg-yellow-400 text-black px-6 py-3 rounded-md text-lg font-semibold hover:bg-gray-300"
          >
            Restart
          </button>
        </div>
      ...
      {startGame && !gameOver && (
        <>
          <div className="absolute top-5 left-5 text-lg font-bold">Score: {score}</div>
          ...
          <div
            className="absolute bg-blue-500 w-12 h-12 rounded-lg"
